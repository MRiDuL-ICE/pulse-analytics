...
async def pageviews(
    ...
    data = await get_pageview_counts(tenant_id, start, end, db)
...
async def top_pages(
    ...
    data = await get_top_pages(tenant_id, start, end, limit, db)
    return {"tenant_id": tenant_id, "data": data}
...
async def event_breakdown(
    ...
    data = await get_event_breakdown(tenant_id, start, end, db)
    return {"tenant_id": tenant_id, "data": data}